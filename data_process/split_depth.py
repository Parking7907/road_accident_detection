    ...
    if i < test_len:
        ...
        for k in true:
            if k.startswith(true_loc + vid_n + '_'):
                ...
                command = ("cp %s /home/data/jinyoung/Server/Demo_depth/Test/True/"%(k))
                output = subprocess.call(command, shell=True, stdout=None)
        
        for l in false:
            if l.startswith(false_loc + vid_n + '_'):
                #print("Test False :", false_loc + vid_n)
                command = ("cp %s /home/data/jinyoung/Server/Demo_depth/Test/False/"%(l))
                output = subprocess.call(command, shell=True, stdout=None)
    else:
        ...
        for k in true:
            if k.startswith(true_loc + vid_n + '_'):
                ...
                command = ("cp %s /home/data/jinyoung/Server/Demo_depth/Train/True/"%(k))
                output = subprocess.call(command, shell=True, stdout=None)
        
        for l in false:
            if l.startswith(false_loc + vid_n + '_'):
                #print("Train False:", false_loc + vid_n)
                command = ("cp %s /home/data/jinyoung/Server/Demo_depth/Train/False/"%(l))
                output = subprocess.call(command, shell=True, stdout=None)
print("T_F STARTS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    ...
    if i < test_len_FT:
        for k in false:
            if k.startswith(false_loc + vid_n + '_'):
                #print("Train False :", false_loc + vid_n)
                command = ("cp %s /home/data/jinyoung/Server/Demo_depth/Test/False/"%(k))
                output = subprocess.call(command, shell=True, stdout=None)
    else:
        for k in false:
            if k.startswith(false_loc + vid_n + '_'):
                #print("Train False :", false_loc + vid_n)
                command = ("cp %s /home/data/jinyoung/Server/Demo_depth/Train/False/"%(k))
                output = subprocess.call(command, shell=True, stdout=None)
