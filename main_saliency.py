...
import pickle
import numpy as np
from model_demo import DEMONET
from model_saliency import MYNET
...
def train(args):
    ...
    if args.dataset == 'CADP':
        from datasets.CADP import CADP
        train_dataset = CADP(config['datasets']['train'], 'train', config['MYNET']['sequence_size'], temporal_stride=config['datasets']['stride'])
        valid_dataset = CADP(config['datasets']['valid'], 'valid', config['MYNET']['sequence_size'])
...
def test(args):
    ...
    if args.dataset == 'CADP':
        from datasets.CADP import CADP
        test_dataset = CADP(config['datasets']['test'], config['MYNET']['sequence_size'])
    #pdb.set_trace()
